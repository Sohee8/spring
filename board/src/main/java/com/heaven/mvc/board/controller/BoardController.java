...
/*
 ...
 첫째로 컨트롤러의 메소드가 생성하는 모델정보 중에서 @SessionAttributes에 지정한 이름에 동일한 것이 있으면 이를 세션에 저장한다.
 ...
 동일할 경우 오브젝트를 생성하지 않고 세션에 잇는 오브젝트를 사용하게 된다.
*/	
	@Autowired
	...
	public String edit(@Valid @ModelAttribute BoardVO boardVO, 
		...
		if(result.hasErrors()) {
			...
			if(boardVO.getPassword() == pwd) {
				...
				sessionStatus.setComplete(); //현재 컨트롤러 세션에 저장된 모든 정보를 제거해준다. 개별적으로 제거할 순 없다.
				return "redirect:/board/list";
			}
			/*@SessionAttributes
			세션에 저장한 모델이 더 이상 필요없어질 경우 세션에서 제거해줘야 한다.
			세션의 제거 시점은 스프링이 알 수 없으므로 이를 제거하는 책임은 컨트롤러에게 있다.
			세션의 경우 서버의 메모리를 사용하므로 필요없는 시점에 제거해주지 않으면 
			메모리 누수가 발생할 수 있기때문에 항상 빼먹지 않고 제거해줘야 한다.
			*/
